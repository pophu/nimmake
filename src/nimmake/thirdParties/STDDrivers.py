...
class STDDrivers(ThirdParty):
    ...
    def setup(self) -> None:
        ...
        log.debug(f"HALDrivers defines: {self.defines}")
    ...
    def __child_dirs(self, cpu: str) -> list[str]:
        ...
        for child_dir in self.dir.iterdir():
            if not child_dir.is_dir():
                continue
            if "_StdPeriph_" not in child_dir.name.upper():
                continue
    ...
    def __add_cmsis(self, cpu: str) -> None:
        ...
        if self.params and "tool" in self.params:
            if self.params["tool"] == "armclang":
                asm_type = "arm"
        stm32_dir = self.dir / "CMSIS" / "Device" / "ST" / f"{cpu[:-4]}xx"

        asm_fname = f"startup_{cpu.lower()}.s"
        asm_path = stm32_dir / "Source" / "Templates" / asm_type / asm_fname
        if not asm_path.is_file():
            log.error(f"HALDrivers: {asm_path} not found")
            return
        self.srcs.append(asm_path)

        incs = [
            self.root / "CMSIS" / "Include",
            stm32_dir / "Include",
        ]
